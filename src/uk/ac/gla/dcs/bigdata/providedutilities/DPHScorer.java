...
public class DPHScorer {
	 ...
	 * 
	 * @param termFrequencyInCurrentDocument // The number of times the query
	 *                                       appears in the document
	 * @param totalTermFrequencyInCorpus     // the number of times the query
	 *                                       appears in all documents
	 * @param currentDocumentLength          // the length of the current document
	 *                                       (number of terms in the document)
	 * @param averageDocumentLengthInCorpus  // the average length across all
	 *                                       documents
	 * @param totalDocsInCorpus              // the number of documents in the
	 *                                       corpus
	 * @return
	 */
	public static double getDPHScore(short termFrequencyInCurrentDocument, int totalTermFrequencyInCorpus,
			int currentDocumentLength, double averageDocumentLengthInCorpus, long totalDocsInCorpus) {
		...
		// calculate DPH score
		double DPHScore = norm * (termFrequencyInCurrentDocument
				* WeightingModelLibrary
						.log((termFrequencyInCurrentDocument * averageDocumentLengthInCorpus / currentDocumentLength)
								* (totalDocsInCorpus / totalTermFrequencyInCorpus))
				+ 0.5d * WeightingModelLibrary.log(2d * Math.PI * termFrequencyInCurrentDocument * (1d - f)));
